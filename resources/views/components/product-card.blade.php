...
<div class="group flex flex-col overflow-hidden rounded-2xl border border-exportani-border bg-white transition hover:-translate-y-1 hover:shadow-xl hover:shadow-exportani-primary/5">
    <a href="{{ route('products.show', $product) }}" class="relative block h-48 overflow-hidden bg-exportani-background">
        @if($product->gambar)
        ...
        <div class="absolute top-3 left-3 flex flex-wrap gap-1.5">
            @if($product->is_recommended)
                <span class="rounded-full bg-amber-50 border border-amber-200/50 px-2.5 py-0.5 text-[9px] font-bold text-amber-800 uppercase tracking-wide shadow-sm">Rekomendasi</span>
            @endif
            @if($product->owner?->is_trusted_petani)
                <span class="inline-flex items-center gap-1 rounded-full bg-exportani-mint/15 text-exportani-accent border border-exportani-mint/20 px-2.5 py-0.5 text-[9px] font-bold uppercase tracking-wide shadow-sm">
        ...
        @if($product->category)
            <div class="absolute top-3 right-3 rounded-full bg-white/90 px-2.5 py-0.5 text-[9px] font-bold text-exportani-accent border border-exportani-border rounded-full backdrop-blur-sm shadow-sm uppercase tracking-wide">
    ...
    <div class="flex flex-1 flex-col p-5">
        <a href="{{ route('products.show', $product) }}" class="min-w-0 flex-1">
            ...
            <p class="mt-0.5 text-xs text-exportani-secondaryText font-medium">{{ $product->owner?->name ?? 'Petani' }}</p>
            <p class="mt-2 text-xl font-bold text-exportani-primary">
                Rp{{ number_format($product->harga, 0, ',', '.') }}
            </p>
            <div class="mt-4 grid grid-cols-2 gap-y-2 text-xs text-exportani-secondaryText font-medium">
        ...
        @if($showFavorite && auth()->check())
            <div class="mt-5 pt-4 border-t border-exportani-border flex items-center justify-between">
                <form method="POST" action="{{ route('favorites.toggle', $product) }}" class="inline m-0">
                    @csrf
                    <button type="submit" class="text-exportani-secondaryText hover:text-rose-500 transition-colors p-1 focus:outline-none">
                ...
                <a href="{{ route('products.show', $product) }}" class="inline-flex items-center text-xs font-bold text-exportani-accent hover:text-exportani-dark transition">
                    Lihat Detail <span aria-hidden="true" class="ml-1 font-sans">&rarr;</span>
                </a>
            </div>
        @endif
    </div>
