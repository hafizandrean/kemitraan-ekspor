<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-4">
            <a href="{{ route('admin.chat.dashboard') }}" class="flex h-10 w-10 items-center justify-center rounded-full bg-stone-100 dark:bg-gray-750 text-stone-500 dark:text-gray-400 hover:bg-emerald-50 dark:hover:bg-emerald-950/20 hover:text-emerald-600 dark:hover:text-emerald-450 transition-colors">
            ...
            <div>
                ...
                <p class="mt-1 text-sm text-stone-600 dark:text-stone-400">Analisis isi percakapan dan putuskan tindakan administratif.</p>
            </div>
        </div>
    </x-slot>
    ...
    <div class="py-10 px-4 sm:px-6 lg:px-8 max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2 space-y-6">
            <div class="bg-white dark:bg-gray-800 border border-stone-200/80 dark:border-gray-700/50 rounded-2xl shadow-sm overflow-hidden flex flex-col h-[600px]">
                <div class="px-6 py-4 bg-stone-50 dark:bg-gray-900/30 border-b border-stone-100 dark:border-gray-700/50 flex items-center justify-between shrink-0">
                    <div>
                        ...
                        @if($report->conversation && $report->conversation->product)
                            <p class="text-[10px] text-stone-500 dark:text-stone-400 mt-0.5">Produk terkait: <span class="font-bold text-emerald-700 dark:text-emerald-400">{{ $report->conversation->product->nama_produk }}</span></p>
                        @endif
                    </div>
                ...
                <div class="flex-1 overflow-y-auto p-6 space-y-4 bg-stone-50/50 dark:bg-gray-900/10">
                    @forelse($messages as $msg)
                        ...
                        <div class="flex flex-col {{ $isReporter ? 'items-start' : 'items-end' }}">
                            ...
                            <div class="max-w-[85%] rounded-2xl px-4 py-2.5 text-xs shadow-sm {{ $isReporter ? 'bg-white dark:bg-gray-700 text-stone-800 dark:text-gray-200 rounded-tl-none border border-stone-200/50 dark:border-gray-650' : 'bg-emerald-600 dark:bg-emerald-700 text-white rounded-tr-none' }}">
        ...
        <div class="space-y-6">
            <!-- Card 1: Detail Aduan -->
            <div class="bg-white dark:bg-gray-800 border border-stone-200/80 dark:border-gray-700/50 rounded-2xl shadow-sm p-6 space-y-4">
            ...
            <div class="bg-white dark:bg-gray-800 border border-stone-200/80 dark:border-gray-700/50 rounded-2xl shadow-sm p-6 space-y-4">
                ...
                @if($report->status === 'pending')
                    <div class="space-y-3 pt-1">
                        <form method="POST" action="{{ route('admin.chat.report.resolve', $report) }}">
                            ...
                            <button type="submit" class="w-full inline-flex items-center justify-center rounded-xl bg-emerald-650 hover:bg-emerald-700 text-white font-bold py-3 text-xs shadow-md transition-colors">
                ...
                @else
                    <div class="rounded-xl p-4 bg-stone-50 dark:bg-gray-900 border border-stone-150 dark:border-gray-750 text-center space-y-1">
                        ...
                        <strong class="text-xs uppercase tracking-widest text-emerald-700 dark:text-emerald-450 font-black">
            ...
            <div class="bg-white dark:bg-gray-800 border border-stone-200/80 dark:border-gray-700/50 rounded-2xl shadow-sm p-6 space-y-4">
                ...
                <div class="space-y-4 pt-1">
                    <div class="flex items-center justify-between text-xs">
                        ...
                        @else
                            <span class="inline-flex items-center rounded-full bg-emerald-100 dark:bg-emerald-950/45 px-2.5 py-0.5 text-xs font-bold text-emerald-800 dark:text-emerald-350 border border-emerald-500/10">Active</span>
                        @endif
                    </div>
                    ...
                    <form method="POST" action="{{ route('admin.users.status.update', $report->reportedUser) }}" class="space-y-2.5">
                        ...
                        <div>
                            ...
                            <select name="status" required class="w-full rounded-xl border border-stone-200 dark:border-gray-700 bg-white dark:bg-gray-900 text-stone-850 dark:text-gray-200 py-2.5 px-3 text-xs font-semibold focus:outline-none focus:ring-1 focus:ring-emerald-600 focus:border-emerald-600">
