    ...
    <div class="py-10 px-4 sm:px-6 lg:px-8">
        <div class="max-w-4xl mx-auto">
            <div class="rounded-2xl border border-stone-200/80 bg-white/90 p-6 sm:p-8 shadow-sm shadow-stone-900/5 backdrop-blur-md">
                <form method="POST" action="{{ route('petani.products.update', $product) }}" class="space-y-6" enctype="multipart/form-data">
                    @csrf
                    @method('PATCH')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="space-y-5">
                            ...
                            <div>
                                ...
                                <select id="kategori_id" name="kategori_id" class="mt-1.5 block w-full border-stone-300 focus:border-emerald-500 focus:ring-emerald-500 rounded-lg shadow-sm" required>
                                    ...
                                    @foreach($categories as $kategori)
                                        <option value="{{ $kategori->id }}" {{ old('kategori_id', $product->kategori_id) == $kategori->id ? 'selected' : '' }}>{{ $kategori->name }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <x-text-input id="harga" name="harga" type="number" min="0" class="mt-1.5 block w-full" :value="old('harga', $product->harga)" required />
                        ...
                        <div class="space-y-5">
                            <div>
                                ...
                                <textarea id="deskripsi" name="deskripsi" rows="5" class="mt-1.5 block w-full border-stone-300 focus:border-emerald-500 focus:ring-emerald-500 rounded-lg shadow-sm" required>{{ old('deskripsi', $product->deskripsi) }}</textarea>
                                <x-input-error class="mt-2" :messages="$errors->get('deskripsi')" />
                            </div>

                            <div>
                                <x-input-label for="gambar" value="Gambar Produk (Kosongkan jika tidak ingin diubah)" />
                                @if($product->gambar)
                                    <div class="mt-2 mb-3">
                                        <img src="{{ Storage::url($product->gambar) }}" alt="{{ $product->nama_produk }}" class="h-32 w-auto object-cover rounded-md border border-stone-200">
                                    </div>
                                @endif
                                <div class="mt-1.5 flex justify-center rounded-lg border border-dashed border-stone-300 px-6 py-6 hover:border-emerald-500 bg-stone-50 transition-colors">
                                    <div class="text-center">
                                        <svg class="mx-auto h-10 w-10 text-stone-400" viewBox="0 0 24 24" fill="currentColor">
                                            <path fill-rule="evenodd" d="M1.5 6a2.25 2.25 0 012.25-2.25h16.5A2.25 2.25 0 0122.5 6v12a2.25 2.25 0 01-2.25 2.25H3.75A2.25 2.25 0 011.5 18V6zM3 16.06V18c0 .414.336.75.75.75h16.5A.75.75 0 0021 18v-1.94l-2.69-2.689a1.5 1.5 0 00-2.12 0l-.88.879.97.97a.75.75 0 11-1.06 1.06l-5.16-5.159a1.5 1.5 0 00-2.12 0L3 16.061zm10.125-7.81a1.125 1.125 0 112.25 0 1.125 1.125 0 01-2.25 0z" clip-rule="evenodd" />
                                        </svg>
                                        <div class="mt-2 flex text-sm leading-6 text-stone-600 justify-center">
                                            <label for="gambar" class="relative cursor-pointer rounded-md bg-white font-semibold text-emerald-600 hover:text-emerald-500 focus-within:outline-none">
                    ...
                    <div class="flex flex-col-reverse gap-3 pt-6 border-t border-stone-200 sm:flex-row sm:justify-end mt-8">
                        ...
                        <x-primary-button class="justify-center bg-emerald-600 hover:bg-emerald-700">Simpan perubahan</x-primary-button>
                    </div>
                </form>
            </div>
        </div>
